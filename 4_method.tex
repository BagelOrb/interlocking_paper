...
\begin{figure*}
	...
	\begin{subfigure}[B]{.33\textwidth}
		...
		\caption{Failures modes for TPLA (in red) and PP (in blue)}
...
The above \crefrange{eq:tensile}{eq:va} combine to form the constrained optimization problem given by \crefrange{eq:objective}{eq:g_z_shear_b}.
...
\begin{tcolorbox}[float,colback=white,title=Straight model]
	\begin{align}
		...
		\gvb: & \lmax - \va \ge \wmin{b} 		&&	 \label{eq:gvb}\\
		...
		\gca: & \frac{\wb}{w} \frac{ F }{ 2 \va 2 \hmin  } \le  \frac{1}{\sqrt{3}} \sigmafail{a} &&	 \mxz{a}  \label{eq:g_shear_a} \\
		\gza: & \frac{\wa}{w} \frac{ F }{ 2 \va \wa  } \le  \frac{1}{\sqrt{3}} \sigmafailz{a} 	&&	 \mxy{a} \label{eq:g_z_shear_a} \\
		\gzb: & \frac{\wb}{w} \frac{ F }{ 2 \left( \lmax - \va \right) \wb  } \le  \frac{1}{\sqrt{3}} \sigmafailz{b} 	&&	\mxy{b} \label{eq:g_z_shear_b}
	\end{align}
\end{tcolorbox}
...
The shear constraints \crefrange{eq:g_tensile_a}{eq:g_z_shear_b} are replaced by \cref{eq:g_shear_b_broken,eq:g_z_shear_a_broken}.
