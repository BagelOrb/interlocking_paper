...
\begin{abstract}
Fused deposition modelling (FDM) has developed over the recent years from a tool used for prototyping to a tool for manufacturing end-use products.
...
In order to affix two bodies of incompatible materials a common strategy is to apply mechanically interlocking structures at the interface where the two bodies meet.

Existing literature is focused on 2D interlocking dovetail structures for FDM.
...
The interlock is therefore less affected by the material's tendency to deform under stress, overcoming a limitation of the dovetail structures.
However, requiring a high genus topology to satisfy extrusion continuity constraints appears challenging.

We propose a simple and effective 3D interlocking microstructure consisting of interlaced horizontal beams in vertically alternating directions which constitute a high genus topology: the interlaced topologically interlocking microstructure (ITIM).
...
In the diagonal orientation ITIM can obtain 82\% of the theoretical upper bound of \SI{8.6}{\mega\pascal}, which is 67\% of the ultimate tensile strength of the PP.
% and slightly higher than the strength obtained using dovetail designs.
If the application design only allows for an interface region less than \SI{2.4}{\milli\meter} long, the straight orientation is expected to outperform the diagonal orientation of the ITIM lattice.
