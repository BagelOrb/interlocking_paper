...
A unit cell of the straight ITIM variant is visualized in \cref{fig:straight_model}.
...
See \cref{fig:straight_model_failure_modes}.
...
Because TPLA breaks at a lower strain than PP (see \cref{tab:mat_props_manufacturing_constraints}), we know that for these materials $\mxz{b}$ never occurs unless $\mxz{a}$ has occurred.
...
The diagonal ITIM model is then defined by the constrained optimization problem given by \crefrange{eq:objective_diagonal}{eq:g_z_shear_m_diag}.
