...
Also the logarithmic sensitivities for the constraint functions were evaluated for constraint $g_1$. Only variable $w_a$ influences constraint $g_{1a}$ and only $w_b$ influences $g_{1b}$.
 % Since $w_a - w_{min}$ and $w_b - w_{min}$ are smaller than $w_a$ and $w_b$ respectively, $\diff{_L g_{1a}}{_L w_a}$ and $\diff{_L g_{1b}}{_L w_b}$ are both larger than 1. Therefore variable $w_a$ is a very influential parameter on $g_{1a}$ and $w_b$ is a very influential parameter on $g_{1b}$.
