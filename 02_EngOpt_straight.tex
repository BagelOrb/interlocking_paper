...
\begin{align*}
	%\frac{ 3 F }{ 4 v^m \hc} &\le \tau^m_\text{fail} \\
	%\frac{ 3 F }{ 4 v^m \hc} &\le \nicefrac1{\sqrt{3}} \sigma^m_\text{fail} \\
	%\frac{ 3 F }{ 4 \cdot \sqrt{3} v^m \hc} &\le \sigma^m_\text{fail} \\
	%\frac{ F w^b }{ 2 \left( v^a \right)^2 \hc } &\le \sigma^a_\text{fail} \\
	%\\
...
\begin{align*}
	%\frac{ F }{ v^a \hc \sigma^a_\text{fail} }  \max{\left( \frac34 \sqrt{3}, \frac{w^b}{2v^a} \right)} \le 1  \bigvee
	%\frac{ F }{ v^b \hc \sigma^b_\text{fail} }  \max{\left( \frac34 \sqrt{3}, \frac{w^a}{2v^b} \right)} \le 1  \\
	%\min{\left( \frac{ F }{ v^a \hc \sigma^a_\text{fail} }  \max{\left( \frac34 \sqrt{3}, \frac{w^b}{2v^a} \right)}
	%, \frac{ F }{ v^b \hc \sigma^b_\text{fail} }  \max{\left( \frac34 \sqrt{3}, \frac{w^a}{2v^b} \right)} \right)} \le 1  \\
	%\frac{F}{\hc}  \min{\left( \frac{ 1 }{ v^a \sigma^a_\text{fail} }  \max{\left( \frac34 \sqrt{3}, \frac{w^b}{2v^a} \right)}
...
\begin{align*}
	...
	\gta: & 1 - \frac{ 2 \wmin^a \hf \sigma^a_\text{fail} }{ F } \le 0 \\
	\gtb: & 1 - \frac{ w^b \hf \sigma^b_\text{fail} }{ F } \le 0 \\
	\gc: & 1- \frac{\hmin}{F}  \max{\left( \frac{ v^a \sigma^a_\text{fail} }{ \max{\left( \frac34 \sqrt{3}, \frac{w^b}{2v^a} \right)} }  
		, \frac{ v^b \sigma^b_\text{fail} }{ \max{\left( \frac34 \sqrt{3}, \frac{2\wmin^a}{2v^b} \right)} }   \right)} \le 0 \\
	\gza: & 1 - \frac{ 4 v^a 2 \wmin^a \tau^a_\text{Z} }{ 3 F } \le 0 \\
	\gzb: & 1 - \frac{ 4 v^b w^b \tau^b_\text{Z} }{ 3 F } \le 0
\end{align*}
...
\begin{align*}
	...
	%\frac{3 \wmin^a}{\hmin \sigma_\text{fail}^a v^a} &= \frac{\hmin}{\left(\hf\right)^2 \sigma_\text{fail}^b} \\
	%\frac{\hmin \sigma_\text{fail}^a v^a}{3 \wmin^a} &= \frac{\left(\hf\right)^2 \sigma_\text{fail}^b}{\hmin} \\
	%\hmin \sigma_\text{fail}^a v^a &= \frac{\left(\hf\right)^2 \sigma_\text{fail}^b  3 \wmin^a  }{  \hmin} \\
	%v^a &= \frac{\hmin \sigma_\text{fail}^a \left(\hf\right)^2 \sigma_\text{fail}^b  3 \wmin^a  }{  \hmin} \\
	%v^a &= \sigma_\text{fail}^a \left(\hf\right)^2 \sigma_\text{fail}^b  3 \wmin^a  \\
