...
\revise{With a relatively small length $\lmax$ it stands to reason to fill that space using a single unit cell in the length direction only.
...
We consider a single layer of ITIL cells along the interface.
Adding more cells in the direction orthogonal to the interface would make them protrude farther into the two bodies, which limits design applicability.
If the two layers of cells would have the same geometry then we would not expect any gain in the ultimate strength of the interlock.
Optimizing the geometry of the two layers of cells separately could lead to improved ultimate strength, but falls beyond the scope of this paper.
}
...
\subsubsection{\revise{}{Straight ITIL variant (whole)}}
...
These three types of failure mode for either material are model\revise{l}{}ed using classical beam theory as having a homogen\revise{}{e}ous stress distribution.
Because the total force $F$ is model\revise{l}{}ed as being homogeneously distributed over the whole cross beam,
...
See \crefrange{interlocking:eq:tensile}{interlocking:eq:z_shear}.
...
\begin{figure}
	\begin{tcolorbox}[colback=white,title=\revise{}{Stresses in the straight ITIL variant}]
\begin{align}
...
Combining the above we obtain the constrained optimization problem given by \crefrange{interlocking:eq:objective}{interlocking:eq:g_z_shear_b}\revise{ in \cref{interlocking:eqs:straight_model}}{}.
...
We will construct a separate model for analy\revise{s}{z}ing the case where the \revise{T}{}PLA cross beam is broken into segments.
...
\begin{figure}
	...
	\begin{subfigure}[B]{.59\columnwidth}
		...
		\caption{Response surface colo\revise{u}{}red to failure mode.}
