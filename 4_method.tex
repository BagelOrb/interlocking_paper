...
\begin{align*}
	...
	F &= F^a + F^b \\
	&\dots \\
	% &= F^a + F^a \frac{E^b w^b}{E^a w^a} \\
	% &= F^a \left( 1 + \frac{E^b w^b}{E^a w^a} \right) \\
	% &= \sigma^a h w^a \left( 1 + \frac{E^b w^b}{E^a w^a} \right) \\
	% &= \sigma^a h \left( w^a + \frac{E^b}{E^a} w^b \right) \\
	% &= \sigma^a h w^a + \sigma^a h \frac{E^b}{E^a} w^b \\
	F^* &= \sigma^a_\text{yield} h \left( w^a + \frac{E^b}{E^a} w^b \right) \\
\end{align*}

So if the total width is constrained $w = w^a + w^b$ then the optimal distribution is $w = w^a$ and $w^b=0$.
