...
\begin{align}
	...
	\frac{ 3 F }{ 4 v^m \hc} &\le \taufail^m					&&\text{ Shear failure } M_y^m  \label{eq:c_shear} \\
	...
	\frac{ 3 F }{ 4 v^m w^m } &\le \tauz^m							&&\text{ Shear failure } M_z^m  \label{eq:c_shear_z} \\
...
\begin{align*}
	%\frac{ 3 F }{ 4 v^m \hc} &\le \taufail^m \\
	%\frac{ 3 F }{ 4 v^m \hc} &\le \nicefrac1{\sqrt{3}} \sigmafail^m \\
	%\frac{ 3 F }{ 4 \cdot \sqrt{3} v^m \hc} &\le \sigmafail^m \\
	%\frac{ F w^b }{ 2 \left( v^a \right)^2 \hc } &\le \sigmafail^a \\
	%\\
	\frac{ F }{ v^a \hc }  \max{\left( \frac34 \sqrt{3}, \frac{w^b}{2v^a} \right)} &\le \sigmafail^a  \\
	\frac{ F }{ v^b \hc }  \max{\left( \frac34 \sqrt{3}, \frac{w^a}{2 v^b} \right)} &\le \sigmafail^b  
\end{align*}
...
\begin{align*}
	\frac{ F }{ v^a \hc }  \max{\left( \frac34 \sqrt{3}, \frac{w^b}{2v^a} \right)} \le \sigmafail^a  &\bigvee \\
	\frac{ F }{ v^b \hc }  \max{\left( \frac34 \sqrt{3}, \frac{w^a}{2v^b} \right)} \le \sigmafail^b  
\end{align*}

The logical disjunction can be rewritten into a minimum when both constraints are transformed into negative null form:
\begin{align*}
	%\frac{ F }{ v^a \hc \sigmafail^a }  \max{\left( \frac34 \sqrt{3}, \frac{w^b}{2v^a} \right)} \le 1  \bigvee
	%\frac{ F }{ v^b \hc \sigmafail^b }  \max{\left( \frac34 \sqrt{3}, \frac{w^a}{2v^b} \right)} \le 1  \\
	%\min{\left( \frac{ F }{ v^a \hc \sigmafail^a }  \max{\left( \frac34 \sqrt{3}, \frac{w^b}{2v^a} \right)}
	%, \frac{ F }{ v^b \hc \sigmafail^b }  \max{\left( \frac34 \sqrt{3}, \frac{w^a}{2v^b} \right)} \right)} \le 1  \\
	%\frac{F}{\hc}  \min{\left( \frac{ 1 }{ v^a \sigmafail^a }  \max{\left( \frac34 \sqrt{3}, \frac{w^b}{2v^a} \right)}
	%, \frac{ 1 }{ v^b \sigmafail^b }  \max{\left( \frac34 \sqrt{3}, \frac{w^a}{2v^b} \right)} \right)} \le 1  \\
	%\frac{F}{\hc}  \min{\left( \frac{ \max{\left( \frac34 \sqrt{3}, \frac{w^b}{2v^a} \right)} }{ v^a \sigmafail^a }  
	%, \frac{ \max{\left( \frac34 \sqrt{3}, \frac{w^a}{2v^b} \right)} }{ v^b \sigmafail^b }   \right)} \le 1  \\
	\frac{F}{\hc}  \min{\left( \frac{ \max{\left( \frac34 \sqrt{3}, \frac{w^b}{2v^a} \right)} }{ v^a \sigmafail^a }  
		, \frac{ \max{\left( \frac34 \sqrt{3}, \frac{w^a}{2v^b} \right)} }{ v^b \sigmafail^b }   \right)} - 1 \le 0  
\end{align*}
...
\begin{align*}
	...
	\gc: & 1- \frac{\hmin}{F}  \max{\left( \frac{ v^a \sigmafail^a }{ \max{\left( \frac34 \sqrt{3}, \frac{w^b}{2v^a} \right)} }  
		, \frac{ v^b \sigmafail^b }{ \max{\left( \frac34 \sqrt{3}, \frac{2\wmin^a}{2v^b} \right)} }   \right)} \le 0 \\
	\gza: & 1 - \frac{ 4 v^a 2 \wmin^a \tauz^a }{ 3 F } \le 0 \\
	\gzb: & 1 - \frac{ 4 v^b w^b \tauz^b }{ 3 F } \le 0
\end{align*}
...
\begin{align*}
	%	\min{ \frac{\left( 2 \wmin^a + w^b \right) \left( \hf + \hmin \right) }{F} }\\
...
\begin{align*}
	...
	%\frac{3 \wmin^a}{\hmin \sigmafail^a v^a} &= \frac{\hmin}{\left(\hf\right)^2 \sigmafail^b} \\
	%\frac{\hmin \sigmafail^a v^a}{3 \wmin^a} &= \frac{\left(\hf\right)^2 \sigmafail^b}{\hmin} \\
	%\hmin \sigmafail^a v^a &= \frac{\left(\hf\right)^2 \sigmafail^b  3 \wmin^a  }{  \hmin} \\
	%v^a &= \frac{\hmin \sigmafail^a \left(\hf\right)^2 \sigmafail^b  3 \wmin^a  }{  \hmin} \\
	%v^a &= \sigmafail^a \left(\hf\right)^2 \sigmafail^b  3 \wmin^a  \\
